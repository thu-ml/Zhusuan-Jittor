...
class Distribution(object):
    """
    The :class:`Distribution` class is the base class for various probabilistic distributions which support batch inputs, generating batches of samples and evaluate probabilities at batches of given values.

    The typical input shape for a :class:`Distribution` is like ``batch_shape + input_shape``. where ``input_shape`` represents the shape of non-batch input parameter, :attr:`batch_shape` represents how many independent inputs are fed into the distribution.

    Samples generated are of shape
    ``([n_samples]+ )batch_shape + value_shape``. The first additional axis is omitted only when passed `n_samples` is None (by default), in which case one sample is generated. :attr:`value_shape` is the non-batch value shape of the distribution. For a univariate distribution, its
    :attr:`value_shape` is [].
    ...
    When evaluating probabilities at given values, the given Var should be broadcastable to shape ``(... + )batch_shape + value_shape``. The returned Var has shape ``(... + )batch_shape[:-group_ndims]``.
    ...
    The value type of `log_prob` will be `param_dtype` which is deduced from the parameter(s) when initializating. And `dtype` must be among `int16`, `int32`, `int64`, `float16`, `float32` and `float64`.

    When two or more parameters are vars and they have different type, `TypeError` will be raised.
    ...
    def _log_prob(self, given):
        """
        Private method for subclass to rewrite the :meth:'log_prob' method.
        """
        raise NotImplementedError()
